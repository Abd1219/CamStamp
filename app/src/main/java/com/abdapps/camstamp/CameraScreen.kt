...
fun CameraScreen(modifier: Modifier = Modifier) {
        ...
        if (lastPhotoUri != null) {
            try {
                thumbnailImageBitmap = withContext(Dispatchers.IO) {
                    localContext.contentResolver.openInputStream(lastPhotoUri!!)?.use {
    ...
    LaunchedEffect(zoomStateValue?.linearZoom) {
        zoomStateValue?.linearZoom?.let { cameraLinearZoom ->
            if (cameraLinearZoom != linearZoom) linearZoom = cameraLinearZoom
        }
    }
    ...
    fun getCurrentLocation() {
        // ... (getCurrentLocation implementation remains the same)
    ...
    fun takePhoto(cameraExecutor: Executor) {
        // ... (takePhoto implementation remains the same)
    ...
    Column(modifier = modifier.fillMaxSize().background(Color.Black)) { // Main container is now a Column
        // Top Bar
        Box(
            modifier = Modifier
                .fillMaxWidth()
                .height(100.dp)
                .background(Color.Black.copy(alpha = 0.5f)) // Keep translucent background for the bar
        ) {
            IconButton(
                onClick = { textFieldValue = customText; showCustomTextDialog = true },
        ...
        Box(
            modifier = Modifier
                .fillMaxWidth()
                .weight(1f) // This Box will take up the remaining space
        ) {
